...
#include <string>
std::string FormatFullVersion() { return "1.0.0-PoWH"; }
 ...
 * between powhd, and powh-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GeneratePoWH(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), POWH_CONF_FILENAME));
    if (mode == HMM_RAVEND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "powhd.pid"));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/dstr1989/PoWH>";
    const std::string URL_WEBSITE = "<https://powh.io>";
...
/** Sanity checks
 *  Ensure that PoWH is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single PoWH process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", POWH_CONF_FILENAME)).string());
    ...
    // Wyłączone wewnętrzne GeneratePoWH
    ...
    strSubVersion = std::string("/PoWH:1.0.0/");
